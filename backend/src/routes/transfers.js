...
async function sendTransfer(req, res) {
  ...
  await withTransferLock([sender.id, receiver.id, currency], async () => {
    ...
    try {
      ...
      await pocketBase.reconcileWalletBalance(senderWallet, 'transfers.send_completed');
      await pocketBase.reconcileWalletBalance(receiverWallet, 'transfers.receive_completed');
    } catch (error) {
